...
const PostLikeHandler = asyncHandler(async (req, res) => {
    ...
    if (existingLike) {
        ...
        return res.status(200).json(new ApiResponce(200, {}, "Post Unliked Successfully"));
        ...
        return res.status(200).json(new ApiResponce(200, {}, "Post Liked Successfully"));
    }
})
...
const getPosts = asyncHandler(async (req, res) => {
    ...
    const postsWithLikesInfo = await Promise.all(posts.map(async (post) => {
        ...
        const postCommentsCount = await Comment.countDocuments({ post: post._id });
        const userLiked = await PostLikes.exists({ post: post._id, user: userId });
